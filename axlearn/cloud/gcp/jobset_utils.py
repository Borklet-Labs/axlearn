...
class A4XHighReplicatedJob(GPUReplicatedJob):
    """Builds a replicated job spec for an a4x-high GPU job, to be used with JobSet API."""
    ...
    def _build_pod(self) -> Nested[Any]:
        ...
        Returns:
            A nested dict corresponding to a k8s Pod template, including the pod metadata and spec.
        """
        cfg: A4XHighReplicatedJob.Config = self.config
        volumes = self._build_volumes()
        annotations = {
            "kubectl.kubernetes.io/default-container": cfg.name,
        }
                ...
                resourceClaims=[
                    {
                        ...
                        "resourceClaimTemplateName": f"{cfg.name}-compute-domain-channel"
                    }
                ],
                affinity={
                    "nodeAffinity": {
...
class A4XMaxReplicatedJob(GPUReplicatedJob):
    """Builds a replicated job spec for an a4x-max GPU job, to be used with JobSet API."""
    ...
    def _build_pod(self) -> Nested[Any]:
        ...
        Returns:
            A nested dict corresponding to a k8s Pod template, including the pod metadata and spec.
        """
        cfg: A4XMaxReplicatedJob.Config = self.config
        volumes = self._build_volumes()
        annotations = {
            "kubectl.kubernetes.io/default-container": cfg.name,
        }
